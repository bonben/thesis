...
Ce chapitre deuxième étudie et vise à exploiter les oscillations de métriques impliquées dans le décodage itératif de turbo codes. 
...
\begin{enumerate}
	...
	\item Les valeurs des LLRs oscillent, croisant ou non le seuil de décision (0). Dans ce cas, l'évolution
	des valeurs des LLRs est d'allure sinusoïdale.
\end{enumerate}
...
un cas d'usage suffisamment large, permettant de tirer des conclusions générales sur tout turbo décodeur  binaire standardisé.
Le choix de 32 
...
pour une trame donnée. Cent trames erronées et cent trames corrigées sont présentées sur cette figure.
...
113. Cela signifie (en considérant la taille de la trame, à savoir 1024) qu'en moyenne $11\%$ des bits sont erronées dans une trame erronée. 
...
est supérieure à la probabilité moyenne si le bit considéré a oscillé 11 fois ou plus. Plus encore, en considérant les bits 
...
Au niveau du taux d'erreur trame, nous pouvons remarquer que l'algorithme SC a des performances moins intéressantes que celles de
...
Dans cette publication, le cadre des simulations consiste en des turbo codes binaire et double binaires à 8 états, de 
...
\begin{figure}[!h]
	...
	\vspace*{.3cm}
	\caption{\label{fig:fsm}Comparaison des performances de décodage entre les algorithme EML-MAP et FSM (recopiés de \cite{cim} et rejoués) avec plusieurs 
...
critère d'arrêt. 
% Enfin, dernière différence dans cette tentative de reproduction des résultats, le turbo décodeur peut 
...
\begin{figure}[!h]
	...
	\vspace*{.3cm}
	\caption{\label{fig:cmp_osc}Comparaison des performances de décodage entre les algorithme EML-MAP, FSM(16) et différents 0FSM(32). Turbo code du standard LTE (K=1504, R=1/3).}
\end{figure}
