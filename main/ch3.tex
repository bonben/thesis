...
\vspace*{2cm}
  ...
  \includegraphics[scale=0.75]{main/ch3_fig/spc_tie}
  \label{fig:spc_tie}
  }
  \vspace*{2cm}
  \caption{Unités matérielles des instructions spécialisées}
\end{figure}
...
\subsection{Instructions spécialisées à parallélisme maximum}
...
L'implémentation de la fonction $f$ est représentée dans la Figure~\ref{fig:f_tie}. Les deux entrées sont les LLRs notés $L_a$ et $L_b$ et la sortie est notée $f(L_a,L_b)$. Souvent, pour réduire le chemin critique, les LLRs sont représentés en \og signe-amplitude \fg : un bit est utilisé pour le signe, et le reste pour la valeur absolue, toujours positive. Dans notre implémentation, les valeurs négatives sont représentées en complément à deux, car il s'agit du mode de représentation utilisé dans les processeurs XTensa.
...
Une alternative est utilisée dans l'implémentation proposée. Tout d'abord, toutes les fonctions élémentaires pour le décodage de code polaires possèdent le même prototype de fonctions dans la description logicielle. Le type de fonction à appliquer ($f$, $g$, $h$, \texttt{R0}, \texttt{R1}, \texttt{SPC}, \texttt{REP}) est passé en paramètre de la fonction, accompagné des adresses auxquelles accéder aux données à lire et écrire. De cette manière, il est possible de créer, un tableau contenant les pointeurs des fonctions à appeler. L'algorithme permettant de déterminer la séquence de fonctions à exécuter prend comme entrée le tableau de bits gelés correspondant au code polaire traité.
 ...
 \begin{table}[hb]
    ...
    \cmidrule(lr){3-3}
    \cmidrule(lr){4-4}
Processeur de base                               &   $7.30.10^6$  &   $3.4$            & $160000$             \\

+ instructions P\textsubscript{max} $f$ et $g$   &   $2.64.10^6$  &   $9.3$            & $210500$             \\

+ instructions P\textsubscript{max} \texttt{R0},
\texttt{R1},\texttt{SPC} et \texttt{REP}         & $534.00.10^3$  &   $ 46$            & $272000$             \\

+ FLIX3                                          & $296.00.10^3$  &   $ 83$            & $311900$             \\

+ instructions \textit{in-register}              & $192.00.10^3$  &   $128$            & $445500$             \\

+ pseudo unrolled                                & $151.00.10^3$  &   $163$            & $445500$             \\
    \end{tabular}
    }}
  \end{table}
...
%   cycles  debit Area (gates)
% Base                                    7.30E+06    3.38E+06    116.00E+03
% instructions spécialisées f & g         2.64E+06    9.34E+06    144.00E+03
% instructions spécialisées REP R0 SPC R1 534.00E+03  46.17E+06   161.00E+03
% FLIX                                    296.00E+03  83.29E+06   222.00E+03
% In-register                             192.00E+03  128.41E+06  524.00E+03
% Unrolling                               151.00E+03  163.28E+06  524.00E+03
