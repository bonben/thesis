...
en un retard de la convergence de décodage. Ce retard peut se calculer et vaut $10\times \log_{10}\left(\frac{K}{K-\text{Taille}_\text{CRC}}\right)$ dB.
...
de 32 bits pour le standard DVB-RCS2. Aussi, suivant la taille de la trame considéré, le retard de convergence induit par le code 
CRC sera plus important lorsque K est petit. Le tableau \ref{tab:crcShift} récapitule l'impact en dB du retard de convergence
...
En revanche, l'impact en terme de taux d'erreur trame, pour une valeur de SNR constante, ne peut être prédéterminé car il 
...
EML-MAP, il est également utilisé en tant que critère d'arrêt. Pour l'algorithme FNC, son emploi est nécessaire. Le rendement exact
