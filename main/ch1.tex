...
Enfin, en 2005, Crozier \textit{et al.} proposent deux méthodes améliorées, découlant de la précédente \cite{crozierDIM}. Une première impulsion est placée à une position $i$ dans la partie systématique du mot de code. Une seconde y est ajoutée à un position $j$. Ensuite le décodage est mené et les distance et multiplicité associées sont relevées de la manière que pour \cite{garelloAllZero}. Dans l'article \cite{crozierDIM} les auteurs considèrent que placer la seconde impulsion dans la zone $\llbracket i; i+2D\rrbracket$, avec D la distance minimale du code estimée est suffisant. Cependant, dans \cite{yocComparisonMethods} il est précisé que la fiabilité de cette méthode est augmentée si $j \in \llbracket i; K-1\rrbracket$. Pis encore, selon nos expérimentations, dans le cadre de turbo codes non circulaire, afin d’approcher au plus près les résultats fournis par la méthode \cite{garellodfree}, imposer $i \in \llbracket 0; K-1\rrbracket$ et $j \in \llbracket i; N\rrbracket$ semblent offrir le meilleur compromis selon des simulations.
	
\paragraph*{Exemples de prévision du plancher d'erreur}
Ainsi, en utilisant la méthode de la double impulsion de Crozier \cite{crozierDIM} ou bien la méthode utilisant les sous-codes contraints de Garello \cite{garellodfree}, nous pouvons calculer les spectres de distance pour les différents turbo codes standardisés.
...
\end{tabular}}
\end{table}
En utilisant ces données et l'équation \ref{eq:uboundfer}, nous pouvons ajouter sur les courbes de performances de la Figure \ref{fig:ferstdlte} la borne de l'union. Il est remarquable que pour les 3 codes considérés, la courbe de performance est très proche de la borne de l'union lorsque les SNR sont suffisamment élevés.
...
Afin d'abaisser ce plancher d'erreur, différentes méthodes peuvent être employées. 
...
Ainsi, hormis ce premier paramètre, le spectre de distance est régi par l'entrelaceur. C'est pour cela que de nombreuses recherches ses sont concentrées dans la recherche d'entrelaceur. De plus, l'entrelaceur n'a pas d'impact sur la complexité du décodage. Néanmoins, des limites existent quand aux distances et multiplicités minimales obtenables. Aujourd'hui, nous avons à disposition des structures d'entrelaceurs (comme évoqué en section \ref{sec:entrelacement}) permettant d'obtenir des distances minimales aussi grandes que possible. Cependant des avancés continuent en considérant la conception d'entrelaceur conjointement à celle de la matrice de poinçonnage. Cette conception conjointe permet d'obtenir des distance minimales très importantes pour des codes à haut rendement \cite{punctureConstrained}.

Cependant, un autre vecteur d'optimisation des performance est situé au niveau du décodeur. En effet, les méthodes itératives sont sous-optimales. Ainsi, plusieurs méthodes de décodages effectuant des opérations calculatoires après un décodage usuel ont été considérées. Ces approchent peuvent résulter en l'obtention de plusieurs séquence d'information probables. Ainsi, l'utilisation d'un code détecteur d'erreurs concaténé en amont du turbo codeur devient nécessaire. Plus encore, la concaténation série de différents codes permet de combler différentes lacunes des turbo codes. 
...
\paragraph{Code BCH}
Dès l'apparition des turbo codes, il a été remarqué que les motifs d'erreurs dans le plancher d'erreur des turbo codes sont constitués de quelques 1 séparés par des 0. Autrement dit, si pour un SNR élevé, le turbo décodeur ne converge pas vers la bonne séquence d'information, alors la séquence décodée est très proche de la bonne séquence. Partant de ce constant, ANdersen propose en 1996 de concaténer un code BCH avec un turbo code \cite{andersenBCH}. Ainsi, après un non décodage du turbo code non fructueux, un décodage dur du code BCH est effectué permettant d'éliminer les erreurs résiduelles. Cette approche résulte en un abaissement du plancher d'erreur (provenant de l'augmentation de la distance minimale du code via la concaténation série) mais la convergence est retardé en raison de la baisse de rendement provenant de l'ajout de redondance. Par construction, le nombre de bits de redondance nécessaire pour qu'un code BCH corrige $t$ erreurs est donné par : \[R_{BCH} = t \times \lceil \log_2 K\rceil.\]
Ainsi, plus K est grand, moins la baisse de rendement est importante. De plus, la complexité d'un décodeur dur BCH possède une complexité raisonnable. Un schéma de codage équivalent a été choisi dans le cadre du standard DVB-S2 où  un code LDPC est concaténé avec un code BCH.

Afin de réduire la perte de rendement, dans \cite{narayananBCH}, une analyse analytique est réalisée afin d’identifier les positions les plus souvent erronées dans la séquence d'information. Ainsi, uniquement ces positions sont protégées par le BCH, impliquant un besoin de redondance moins important.

Il est à noter que le code BCH peut aussi servir de code détecteur d'erreur et donc de critère d’arrêt. Ceci permet à \cite{takeshitaBCH} d'augmenter le nombre d'itérations du turbo décodeur arguant le fait que le nombre d'itérations moyen n'est que peu impacté. Ceci a pour effet d'améliorer la convergence.

\paragraph{Code RSC de rendement 1}
Le principe d'une concaténation basée sur un tel code est d'encoder partiellement soit l'information redondantes provenant des codeurs élémentaires soit l'information systématique avant de la fournir aux codeurs élémentaires.

Le premier cas de concaténation est nommé turbo codes 3D. Ils ont été pensé par Berrou \textit{et al.} en 2007 \cite{TC3D_1}, puis analysés successivement dans \cite{TC3D_2} et \cite{TC3D_3}.
Le second cas est nommé quant à lui turbo code pré-codé. Leur apparition remonte à 2011 \cite{precoded}. Leur structure a été modifié puis optimisé dans \cite{precoding}.

Que ce soit pour les turbo codes 3D ou les turbo codes pré-codés, l'utilisation d'un code RSC de rendement 1 permet de ne pas modifier le rendement global du schéma de codage. Cependant, plus la part d'information passant par le codeur additionnel est important, plus la convergence est retardée et plus la distance minimale est augmentée. Ainsi, un compromis doit être trouvé suivant les performances visées. 

Dans les deux cas, 3 décodeurs SISO doivent échanger mutuellement de l'information. La complexité calculatoire est donc augmentée. Dans \cite{TC3D_2}, une hausse d'environ $10\%$ est annoncée. Cependant, la latence du système est augmentée puisque une itération de décodage correspond à l'activation successives des 3 décodeurs SISO.

\paragraph{Code CRC}
La troisième concaténation utilise les codes détecteurs d'erreur CRC. Ils possèdent l'avantage d'être souvent définis dans les différents standards employant des turbo codes. Ils servent alors à des demandes de renvoi automatique (ARQ), permettant d'améliorer la qualité de service. Ils peuvent aussi permettre d'arrêter le processus itératif dès lors que le turbo décodeur a convergé vers le bon mot de code. Des analyses ont été menées afin de déterminer les probabilités de détection correction, d'erreur non détectée et de fausse alarme lorsque le décodage du CRC est effectué lors du processus itératif \cite{detectionCRC}. 

\subparagraph{Décodage par liste} Cette famille de décodage a été pensée des la fin des années 50 indépendamment par Elias et Wozencraft \cite{elias1957list,wozencraft1958list}. Dans le cadre des codes convolutifs, son principe est de fournir les $\cal{L}$ séquences les plus probables correspondant à des chemins dans le treillis. Ces $\cal{L}$ séquences peuvent être obtenues grâce à l'algorithme de Viterbi par liste (LVA) possédant une version parallèle et une version séquentielle \cite{lva}. Plus récemment, une dérivation de l'algorithme ML-APP permettant d'obtenir ces $\cal{L}$ séquences a été proposé \cite{mlla}.
Dans le cadre du décodage des turbo codes par listes, trois principales approches peuvent être appliquées :
\begin{itemize}
	\item Chacun des codes constituants est décodé selon un algorithme liste. Dès que l'intersection des deux ensembles de liste contient un élément, le décodage s'arrête et cette séquence est la sortie du décodeur. Cette technique semble avoir moins d'intérêt pour les grandes tailles de trame \cite{sadowPair}. 
	\item Le LVA peut aussi être réalisé après le processus itératif usuel. Dans ce cas, il est exécuté à la fin du processus ou après chaque itération \cite{narayaList}. Les informations \textit{a posteriori} du turbo décodeur servent alors d'information \textit{a priori} pour le LVA. Afin de déterminer la bonne séquence parmi les $\cal{L}$ séquences obtenues, un code détecteur d'erreurs est utilisé. Grâce à cette technique des gains notables dans le plancher d'erreur sont obtenus.
	\item Le décodage fournissant une liste de séquence peut aussi être appliqué avant le processus itératif. Il doit alors fournir $\cal{L}$ séquences distinctes de sorties souples qui seront utilisés en tant qu'information \textit{a priori} par $\cal{L}$ turbo décodeurs distincts. A nouveau, l'identification de la bonne séquence est réalisée par un code détecteur d'erreur. Cette méthode propose des gains aussi bien dans la zone de convergence que dans le plancher d'erreur mais possède un surcoût calculatoire important \cite{newList}.
\end{itemize}

\subparagraph{Décodage utilisant le décodage par statistique ordonnées (OSD)} Des approches utilisant un tel décodage ont été proposé pour les turbo codes \cite{osdAided}. Le principe réside dans le fait d'appliquer l'OSD \cite{osd} de rang 1 après chaque itération de turbo décodage ou après le processus itératif. Le décodage OSD se base sur la matrice génératrice du code. Celle-ci est permutée selon la fiabilité des informations. Grâce à une élimination Gaussienne et en inversant les décisions dures du décodeur turbo, <$K$ mots sont identifiées. Finalement, grâce à un code détecteur d'erreur, le bon mot est identifié. D'importants gains sont obtenus dans la convergence pour de petites tailles de trames et des hauts rendements.  
Cette méthode a été étendue en considérant la matrice globale de la concaténation série du code CRC et du turbo code dans \cite{osdCrcAided}. Dans ce cas le code CRC perd son pouvoir de correction mais les performances de décodage sont améliorées car la distance minimale du code concaténé est utilisé. Cependant, la complexité calculatoire de cette méthode est telle qu'uniquement des décodages de petites tailles de trames peuvent être envisagés.

\subparagraph{Multiples décodages turbo} Cette méthode a été présenté par Crozier et Ould-Cheikh-Mouhamedou dans \cite{cim} puis reformulé dans \cite{fsm}. Son principe est le suivant. Si, à la fin du processus itératif, le décodeur n'a pas convergé vers le bon mot de code, les valeurs absolues des informations \textit{a posteriori} sont triées par ordre croissant. La valeur de l'information canal systématique correspondant à la position la moins fiable est alors forcée à une valeur supérieure à la distance minimale du code. Un nouveau décodage itératif est alors effectué. Le résultat est vérifié grâce à un code détecteur. Si le bon mot n'a pas été identifié, alors, la seconde position la moins fiable est forcée à une valeur supérieure à la distance minimale du code, tout en ayant remis la position la moins fiable à sa valeur originelle. Un nouveau décodage itératif est alors effectué. Ce processus est répété jusqu'à ce qu'un nombre prédéfini de positions testées soit atteint ou jusqu'à ce que le code détecteur valide le mot.
Dans le cadre des turbo codes double binaires, les 4 possibilités de valeurs pour le couple $(A_i,B_i)$ sont testées. Les performances de décodage sont améliorées à la fois dans la convergence et dans le plancher d'erreur mais ce au prix, d'une multiplication de la complexité calculatoire par le nombre de positions examinées.

Cette technique a été étendue par Pfletschinger dans \cite{pflet}. Cette fois toutes les informations provenant du canal sont considérés pour être fixés à une valeur supérieure à la distance minimale du code. Ainsi, jusqu'à $N$ décodages successifs sont effectués. Afin d'obtenir les meilleures performances de décodage le LVA est conjointement employé. Cependant, bien que cette méthode présente les meilleures performances de décodage, elle ne peux être considéré dans un contexte temps réel du fait de sa latence.

\subsubsection{Récapitulatif des différentes méthodes}
Dans cette section, les différentes méthodes sont récapitulées et comparées en terme de gains de décodage, de complexité de décodage et d'implications sur la latence du décodeur. Ces informations sont regroupées dans le tableau \ref{tab:recap}.

\begin{table}[]
\centering
\caption{Récapitulatif des différentes méthodes améliorant les performances de décodage}
\label{tab:recap}
\resizebox{\textwidth}{!}{%
...
\textbf{Localisation des gains}    &  &  &  &  &  \\
\textbf{Nécessité  code détecteur} &  &  &  &  & \\
\textbf{Complexité}                &  &  &  &  & \\
\bottomrule
\end{tabular}}
\end{table}
...
\section{Conclusion}
Dans ce premier chapitre, les notions permettant d'aborder le codage canal ont été présentées. Ensuite, la construction et le décodage et les performances des turbo codes ont été détaillés. Des outils permettant d'estimer les performances d'un turbo code sur le canal BI-AWGN ont été présentés. Finalement, plusieurs méthodes permettant d'améliorer les performances de décodage des turbo codes ont été rapportées. Dans le cadre de turbo codes déjà standardisés, uniquement les méthodes modifiant le processus de décodage peuvent être employées. Puisque le décodage des turbo codes est sous optimal, des optimisations sont encore possibles. D'autant plus, de nombreux standards de communications numériques présentent un code CRC concaténé avec le turbo code. Ceci permet d'envisager une plus grande diversité de décodage. Dans le chapitre suivant, des techniques de décodage tirant parti des oscillations au sein du turbo décodeur sont proposées.