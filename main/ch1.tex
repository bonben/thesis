...
En posant deux théorèmes fondamentaux, Claude Shannon ouvre la voie à la théorie de 
...
%Les $2^k$ éléments $c$ sont appelés mots de code. 
...
vraisemblable, c'est-à-dire celui qui est à la distance de Hamming minimale du mot de code reçu $r$. Cela revient à trouver 
...
%C'est pourquoi la fermeture du treillis est nécessaire de part son impact non négligeable sur les performances de décodage. 
...
associés diffèrent. Généralement, les codeurs récursifs ont des propriétés de distance plus favorables que les codeurs
non-récursifs.
...
deux types de décodeurs sont basés sur l'exploration du treillis associé au code. Le premier est le décodeur à maximum de 
...
AWGN non contraint. Ces courbes sont les limites du rapport signal à bruit à partir desquelles des communications sans 
...
\begin{figure}[!h]
	...
	et pour le canal AWGN à entrée binaire.}
\end{figure}
...
\subsection{La construction des turbo codes}
...
complexité calculatoire croît pronominalement avec la taille du code.
...
Finalement, une bonne permutation doit permettre d'obtenir un écart suffisant afin de minimiser la corrélation entre les 
...
\end{itemize}
Récemment, il a été montré que ces 3 trois types d'entrelaceurs sont équivalents \cite{EquivalenceInt}. En effet, les 
...
\paragraph*{Calcul des probabilités \textit{a posteriori}} L'équation \ref{eq:maprule} permet d'obtenir la décision dure 
...
Ainsi, lors du décodage itératif d'un turbo code (cf. Figure \ref{fig:turboDec}), le décodeur $SISO_1$ reçoit le couple 
...
propriétés de ces turbo codes. Les treillis associés aux codeurs RSC élémentaires des turbo codes de chacun de ces standards 
...
\begin{table}[b]
	...
	\begin{tabular}{@{}rllll@{}}
		...
		\cmidrule(l){2-2}\cmidrule(l){3-5}%\cmidrule(l){4-4}\cmidrule(l){5-5}
...
\begin{figure}[h]
	\begin{center}
		\subfloat[DVB-RCS - K=440]{
			\input{main/ch1_fig/std/dvb440.tikz}
			\label{sub:dvbrcs440}
		}	   

		\subfloat[DVB-RCS - K=752]{
			\input{main/ch1_fig/std/dvb752.tikz}
			\label{sub:dvbrcs752}
		}
...
\begin{figure}[h]
\centering
			\input{main/ch1_fig/std/ccsds.tikz}	
...
\centering
		...
		\input{main/ch1_fig/std/dvb2.tikz}	
		\caption{Taux d'erreur trame pour le standard DVB-RCS2. 
		K=752, différents rendements.
		Décodage par l'algorithme EML-MAP effectuant 
...
Ainsi, $m(\mathbf{\hat{c}})$ et $m(\mathbf{0})$ sont séparés par la distance (Euclidienne) $d_E = 2\sqrt{wE_c} = 2\sqrt{wRE_b}$. 
...
En 2001, un algorithme basé sur l'utilisation de sous codes contraints a été proposé par Garello \textit{et al.} \cite{garellodfree}. 
...
\paragraph*{Exemples de prévision du plancher d'erreurs}
...
Parmi les divers codes qu'il est possible de concaténer avec un turbo code, trois ont particulièrement été considérés 
...
Afin de réduire la perte de rendement, une analyse empirique est réalisée dans \cite{narayananBCH}. Cette dernière 
...
Dans un but d'améliorer les performances de décodage, les codes CRC ont été utilisés de diverses manières. Celles-ci sont 
...
\subparagraph{Décodage par liste} Cette famille de décodage a été pensée des la fin des années 50 indépendamment par Elias 
...
\cite{cim} puis reformulée dans \cite{fsm}. Son principe est le suivant. Si, à la fin du processus itératif, le décodeur 
...
l'adjonction d'un code CRC au turbo code. Des gains de performance sont observés dans le plancher d'erreurs mais la complexité 
...
\begin{tabular}[r]{@{}r@{}}\textbf{Redondance} \\ \textbf{ajoutée}\end{tabular}        & Oui  & Non & CRC & CRC & CRC \\
\begin{tabular}[r]{@{}r@{}}\textbf{Localisation} \\ \textbf{des gains}\end{tabular}    & Plancher & Plancher & Plancher &\begin{tabular}[r]{@{}r@{}}Convergence \\ et plancher\end{tabular}  & \begin{tabular}[r]{@{}r@{}}Convergence \\ et plancher\end{tabular}  \\
\begin{tabular}[r]{@{}r@{}}\textbf{Complexité} \\ \textbf{calculatoire}\end{tabular}                   & \thumbsup & \thumbsmid & \thumbsmid & \thumbsdown  & \thumbsdown \\
\bottomrule
\end{tabular}}
\end{table}
...
%                   \cmidrule(l){2-2}\cmidrule(l){3-3}\cmidrule(l){4-4}\cmidrule(l){5-5}\cmidrule(l){6-6}
% \textbf{Redondance ajoutée}         & Oui  & Non & CRC & CRC & CRC \\
% \begin{tabular}[r]{@{}r@{}}\textbf{Gains dans le zone} \\ \textbf{du plancher d'erreurs}\end{tabular}     & Oui & Oui & Oui & Oui & Oui \\
% \begin{tabular}[r]{@{}r@{}}\textbf{Gains dans le zone} \\ \textbf{de convergence}\end{tabular}     & Non & Non & Non & Oui & Oui \\
% \textbf{Complexité}                 & \thumbsup & \thumbsmid & \thumbsmid & \thumbsdown  & \thumbsdown \\
% \bottomrule
% \end{tabular}}
% \end{table}
